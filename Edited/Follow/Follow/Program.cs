        ...
        private static void onupdate(EventArgs args)
        {
            ...
            if ((Environment.TickCount - time) / 1000 > 40)
            {
                foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(ally => ally.Team == ObjectManager.Player.Team))
                {
                    if (dem)
                    {
                        Obj_AI_Hero hero = ObjectManager.Get<Obj_AI_Hero>().Where(tur => tur.IsAlly && !tur.IsMe && !tur.IsDead).OrderBy(tur => tur.Distance(ObjectManager.Player.ServerPosition)).First();
