...
namespace ANSkarner
{
    class Program
    {
        ...
        private static void KS()
        {
            ...
            {
                if (Q.IsReady() && hero.Distance(ObjectManager.Player) <= Q.Range && Damage.GetSpellDamage(ObjectManager.Player, hero, SpellSlot.Q) >= hero.Health)
                {
                    Q.CastOnUnit(hero, AN.Item("useQKS").GetValue<bool>());

                }

                if (E.IsReady() && hero.Distance(ObjectManager.Player) <= E.Range && Damage.GetSpellDamage(ObjectManager.Player, hero, SpellSlot.E) >= hero.Health)
                {
