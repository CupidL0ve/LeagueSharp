    ...
    internal class Program
    {
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (Damage.GetSpellDamage(ObjectManager.Player, target, 0) > (double)target.Health)
                    {
                        Q.CastOnUnit(target, true);
                    }
                }
            ...
            if (useQ && Q.IsReady())
            {
                foreach (var minion in allMinions)
                {
                    if (minion.IsValidTarget() &&
                        ...
                        0.75 * Damage.GetSpellDamage(ObjectManager.Player, minion, SpellSlot.Q))
                    {
                        Q.CastOnUnit(minion);
                        return;
                    }
            ...
            else if (useE && E.IsReady())
            {
                foreach (var minion in allMinions)
                {
                    if (minion.IsValidTarget(E.Range) &&
                        minion.Health < 0.75 * Damage.GetSpellDamage(ObjectManager.Player, minion, SpellSlot.E))
                    {
                        E.CastOnUnit(minion);
                        return;
                    }
