...
namespace D_Corki
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            _config.SubMenu("Combo").AddItem(new MenuItem("UseWHE", "Your HP% Use W >").SetValue(new Slider(65, 1, 100)));
            ...
            //Misc
            _config.AddSubMenu(new Menu("HitChange", "HitChange"));
            _config.SubMenu("HitChange")
        ...
        private static void Combo()
        {
            ...
            if (useQ && _q.IsReady())
            {
                ...
                if (t != null && t.Distance(_player.Position) < _q.Range && _q.GetPrediction(t).Hitchance >= Qchange())
                    _q.Cast(t, Packets(), true);
            }
            if (useW && _w.IsReady() && usewhE &&
                ObjectManager.Get<Obj_AI_Hero>().Count(hero => hero.IsValidTarget(_r.Range)) <= _config.Item("EnemyC").GetValue<Slider>().Value)
            {
            ...
            if (useR && _r.IsReady())
            {
                ...
                if (t != null && t.Distance(_player.Position) < _r.Range && _r.GetPrediction(t).Hitchance >= Rchange())
        ...
        private static void Laneclear()
        {
            ...
            if (_r.IsReady() && useRl && rlimL < UltiStucks())
            {
        ...
        private static void KillSteal()
        {
            ...
            {
                var t = SimpleTs.GetTarget(_q.Range, SimpleTs.DamageType.Magical);
                if (_q.GetDamage(t) > t.Health && _player.Distance(t) <= _q.Range && _q.GetPrediction(t).Hitchance >= Qchange())
                    _q.Cast(t, Packets(), true);
            }
            if (_e.IsReady() && _config.Item("UseEM").GetValue<bool>())
            {
                var t = SimpleTs.GetTarget(_w.Range, SimpleTs.DamageType.Magical);
                if (_e.GetDamage(t) > t.Health && _player.Distance(t) <= _e.Range && _e.GetPrediction(t).Hitchance >= Echange())
                    _e.Cast(t, Packets(), true);
            }
                        ...
                        ObjectManager.Get<Obj_AI_Hero>()
                            .Where(
                                hero =>
                                    ...
                                    if (_r.GetPrediction(t).Hitchance >= Rchange())
                    _r.Cast(t, Packets(), true);
            }
        }
