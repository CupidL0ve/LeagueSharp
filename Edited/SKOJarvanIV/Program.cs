...
namespace SKOJarvanIV
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args) 
        {
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("ActiveJump", "Jump key").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
        ...
        private static void Combo(Obj_AI_Hero target) 
        {
            if (target != null) {
                ...
                if (Config.Item("UseQCombo").GetValue<bool>() && Q.IsReady() && !E.IsReady())
                {
                    Q.Cast(target, true);
                }
        ...
        private static void Jump()
        {
            ...
            {
                Vector3 pos = Player.ServerPosition + Vector3.Normalize(Game.CursorPos - Player.ServerPosition) * 690;
                E.Cast(pos);
                Q.Cast(pos.To2D());
