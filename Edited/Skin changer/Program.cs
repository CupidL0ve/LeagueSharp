...
namespace Skinchange
{
    class Program
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("SkinChanger").AddItem(new MenuItem("CycleSkins", "CycleSkins!").SetValue(new KeyBind("9".ToCharArray()[0], KeyBindType.Press,false)));
        ...
        public static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (cycleSkins)
            {
                if (canChange)
                {
                    ...
                    {
                        currSkinId = 0;
                        GenerateSkinPacket(ObjectManager.Player.SkinName,0);
                    }
                }
            }
            else
